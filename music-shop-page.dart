...
class _MusicShopPageState extends State<MusicShopPage> {
  ...
  Future<void> _loadLocalAssetSongs() async {
    try {
      ...
      // فیلتر کردن فقط فایل‌های mp3 داخل assets/audio
      final audioFiles = manifestMap.keys
          .where((String key) =>
      key.startsWith('assets/audio/') && key.endsWith('.mp3'))
          .toList();

      print('Found asset audio files: $audioFiles'); // پیام debug
                      ...
                      child: Row(
                        children: [
                          ...
                          ClipRRect(
                            ...
                            child:
                            song.imagePath.startsWith('assets/')
                                ? Image.asset(
                              song.imagePath,
                              width: 44,
                              height: 44,
...
class _SongDetailPageState extends State<SongDetailPage> {
  ...
  Future<String?> _copyAssetToDownloads(
      String assetPath,
      String fileName,
      ) async {
    try {
          ...
          children: [
            Center(
              child: ClipRRect(
                ...
                child:
                widget.song.imagePath.startsWith('assets/')
                    ? Image.asset(
